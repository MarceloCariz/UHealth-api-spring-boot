...
public class UserController {
    ...
    public ResponseEntity<?> getUserInfo(@RequestHeader("Authorization") String token) {
        ...
        if (userInfo != null) {
            ...
            String refreshedToken = JwtUtils.createToken(id, username, email, roles, role);
    ...
    public ResponseEntity<?> getUserProfile(@PathVariable String userId){
        if(userId == null){
            return ResponseEntity.badRequest().body("El id del usuario no puede estar vacio");
        }
    ...
    @PutMapping("/{userId}")
    public ResponseEntity<?>  updateUser(@PathVariable String userId,@Valid  @RequestBody UserRequestUpdate userToUpdate, BindingResult result){
        if(result.hasErrors()) {
    ...
    public ResponseEntity<?> updateRoutine(@PathVariable String routineId, @Valid @RequestBody RoutineRequestUpdate routineRequestUpdate,  BindingResult result){
        if(result.hasErrors()) {
            // manejar errores de validación
            return new ResponseEntity<>(result.getFieldErrors().get(1).getDefaultMessage(), HttpStatus.BAD_REQUEST);
        }
    ...
    public ResponseEntity<?> getAllRoutinesByDate(@Valid @RequestBody RoutineRequestGetByDate routineRequestGetByDate,  BindingResult result){
        if(result.hasErrors()) {
            // manejar errores de validación
            return new ResponseEntity<>(result.getFieldErrors().get(1).getDefaultMessage(), HttpStatus.BAD_REQUEST);
        }
